  ...
  function createCoordinator() {
    ...
    return {
      ...
      positionBadges() {
        ...
        // Sort by descending y so we process badges from the bottom of the screen upward.
        // A single sweep per badge (checking all previously settled badges above it) is
        // sufficient for the common case. We stop scanning earlier badges as soon as the
        // y-gap makes overlap impossible, keeping average complexity well below O(n²).
        placed.sort((a, b) => b.y - a.y);
        for (let i = 1; i < placed.length; i++) {
          const cur = placed[i];
          // Walk backward through already-settled badges (lower y = higher on screen).
          for (let j = i - 1; j >= 0; j--) {
            const prev = placed[j];
            // prev.y <= cur.y (sorted). If prev is already far above cur there is no overlap.
            if (cur.y - (prev.y + prev.height + GAP_PX) > 0) break;
            const xOverlap =
              cur.x < prev.x + prev.width + GAP_PX &&
              cur.x + cur.width + GAP_PX > prev.x;
            if (!xOverlap) continue;
            const yOverlap =
              cur.y < prev.y + prev.height + GAP_PX &&
              cur.y + cur.height + GAP_PX > prev.y;
            if (yOverlap) {
              // Push cur upward so its bottom edge clears prev's top edge.
              cur.y = Math.max(pad, prev.y - cur.height - GAP_PX);
